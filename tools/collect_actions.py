...
def get_json(url, ctx):
    # sirus.su падает — при отказе сразу пробуем зеркало sirus.org (сетевая ошибка = хост мёртв)
    for host in ("https://sirus.su", "https://sirus.org"):
        u = url.replace("https://sirus.su", host)
        for attempt in range(1, 5):
            try:
                with urllib.request.urlopen(urllib.request.Request(u, headers=HEADERS), timeout=25, context=ctx) as r:
                    return json.loads(r.read().decode("utf-8", "replace"))
            except urllib.error.HTTPError as e:
                if e.code != 429 and e.code < 500:
                    break  # не ретраится — к зеркалу
            except Exception:
                break  # сеть/DNS — хост недоступен, сразу к зеркалу
